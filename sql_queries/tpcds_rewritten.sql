...
ssr AS
    ...
    FROM 
        ...
        FROM store_returns 
        WHERE sr_returned_date_sk IN (SELECT d_date_sk FROM date_filtered))
    salesreturns JOIN store ON store_sk = s_store_sk
    GROUP BY s_store_id),
...
csr AS 
    (SELECT cp_catalog_page_id, 
        ...
        FROM (SELECT cs_catalog_page_sk AS page_sk, 
            ...
            WHERE cr_returned_date_sk IN (SELECT d_date_sk FROM date_filtered)) salesreturns
...
wsr AS 
    ...
    SUM(return_amt) AS "returns", 
        ...
        FROM (SELECT ws_web_site_sk AS wsr_web_site_sk, 
            ...
            FROM web_returns LEFT JOIN web_sales ON (wr_item_sk = ws_item_sk AND wr_order_number = ws_order_number)
            WHERE wr_returned_date_sk IN(SELECT d_date_sk FROM date_filtered)) salesreturns
...
FROM 
    ...
    SELECT 'web channel' AS channel, 
        ...
        "returns", 
        (profit - profit_loss) AS profit 
    FROM wsr) x GROUP BY ROLLUP (channel, id) ORDER BY channel, id LIMIT 100;
...
-- Description:
-- =================================================================
select i_brand_id brand_id,i_brand brand,i_manufact_id,i_manufact,sum(ss_ext_sales_price) ext_price from date_dim,store_sales,item,customer,customer_address,store where d_date_sk = ss_sold_date_sk and ss_item_sk = i_item_sk and i_manager_id=14 and d_moy=11 and d_year=2002 and ss_customer_sk = c_customer_sk and c_current_addr_sk = ca_address_sk and substr(ca_zip,1,5) <> substr(s_zip,1,5) and ss_store_sk = s_store_sk group by i_brand,i_brand_id,i_manufact_id,i_manufact order by ext_price desc,i_brand,i_brand_id,i_manufact_id,i_manufact limit 100  
...
-- Description:
-- =================================================================
select i_item_id,i_item_desc,s_store_id,s_store_name,max(ss_quantity) as store_sales_quantity,max(sr_return_quantity) as store_returns_quantity,max(cs_quantity) as catalog_sales_quantity from store_sales,store_returns,catalog_sales,date_dim d1,date_dim d2,date_dim d3,store,item where d1.d_moy = 4 and d1.d_year = 1998 and d1.d_date_sk = ss_sold_date_sk and i_item_sk = ss_item_sk and s_store_sk = ss_store_sk and ss_customer_sk = sr_customer_sk and ss_item_sk = sr_item_sk and ss_ticket_number = sr_ticket_number and sr_returned_date_sk = d2.d_date_sk and d2.d_moy between 4 and 4 + 3 and d2.d_year = 1998 and sr_customer_sk = cs_bill_customer_sk and sr_item_sk = cs_item_sk and cs_sold_date_sk = d3.d_date_sk and d3.d_year in (1998,1998+1,1998+2) group by i_item_id,i_item_desc,s_store_id,s_store_name order by i_item_id,i_item_desc,s_store_id,s_store_name limit 100 
...
-- Description:
-- =================================================================
 WITH store_customers AS ( SELECT DISTINCT c_last_name, c_first_name, d_date FROM store_sales JOIN date_dim ON store_sales.ss_sold_date_sk = date_dim.d_date_sk JOIN customer ON store_sales.ss_customer_sk = customer.c_customer_sk WHERE d_month_seq BETWEEN 1202 AND 1213 ), catalog_customers AS ( SELECT DISTINCT c_last_name, c_first_name, d_date FROM catalog_sales JOIN date_dim ON catalog_sales.cs_sold_date_sk = date_dim.d_date_sk JOIN customer ON catalog_sales.cs_bill_customer_sk = customer.c_customer_sk WHERE d_month_seq BETWEEN 1202 AND 1213 ), web_customers AS ( SELECT DISTINCT c_last_name, c_first_name, d_date FROM web_sales JOIN date_dim ON web_sales.ws_sold_date_sk = d_date_sk JOIN customer ON web_sales.ws_bill_customer_sk = customer.c_customer_sk WHERE d_month_seq BETWEEN 1202 AND 1213 ) SELECT COUNT(*) FROM ( SELECT * FROM store_customers EXCEPT SELECT * FROM catalog_customers EXCEPT SELECT * FROM web_customers ) AS cool_cust 
